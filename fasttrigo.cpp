#ifndef FASTTRIGO_HPP
#define FASTTRIGO_HPP
...
__m256 FTA::length_ps(__m256 x, __m256 y){
    return FTA::sqrt_ps(_mm256_add_ps(_mm256_mul_ps(x,x),_mm256_mul_ps(y,y)));
}

__m256 FTA::length_ps(__m256 x, __m256 y, __m256 z){
    return FTA::sqrt_ps(_mm256_add_ps(_mm256_add_ps(_mm256_mul_ps(x,x),_mm256_mul_ps(y,y)),_mm256_mul_ps(z,z)));
}


__m256 FTA::atan_ps(__m256 x){
    __m256 u=_mm256_mul_ps(x,x);
    __m256 u2=_mm256_mul_ps(u,u);
    __m256 u3=_mm256_mul_ps(u2,u);
    __m256 u4=_mm256_mul_ps(u3,u);
    //__m128 f=1.f+0.33288950512027f*u-0.08467922817644f*u2+0.03252232640125f*u3-0.00749305860992f*u4;

    __m256 f=_mm256_add_ps(_mm256_add_ps(_mm256_add_ps(_mm256_add_ps(_mm256_set1_ps(1.f),
    _mm256_mul_ps(_mm256_set1_ps(0.33288950512027f),u)),
    _mm256_mul_ps(_mm256_set1_ps(-0.08467922817644f),u2)),
    _mm256_mul_ps(_mm256_set1_ps(0.03252232640125f),u3)),
    _mm256_mul_ps(_mm256_set1_ps(-0.00749305860992f),u4));
    return _mm256_div_ps(x,f);
}

__m256 FTA::atan2_ps(__m256 y, __m256 x){
    //__m128 absxgreaterthanabsy=_mm_cmpgt_ps(_mm_andnot_ps(SIGNMASK,x),_mm_andnot_ps(SIGNMASK,y));
    __m256 absxgreaterthanabsy=_mm256_cmp_ps(_mm256_andnot_ps(AVX_SIGNMASK,x),_mm256_andnot_ps(AVX_SIGNMASK,y), _CMP_GT_OQ);
    __m256 ratio=_mm256_div_ps(_mm256_add_ps(_mm256_and_ps(absxgreaterthanabsy,y),_mm256_andnot_ps(absxgreaterthanabsy,x)),
                            _mm256_add_ps(_mm256_and_ps(absxgreaterthanabsy,x),_mm256_andnot_ps(absxgreaterthanabsy,y)));
    __m256 atan=FTA::atan_ps(ratio);

    __m256 xgreaterthan0=_mm256_cmp_ps(x,_mm256_set1_ps(0.f), _CMP_GT_OQ);
    __m256 ygreaterthan0=_mm256_cmp_ps(y,_mm256_set1_ps(0.f), _CMP_GT_OQ);

    atan=_mm256_xor_ps(atan,_mm256_andnot_ps(absxgreaterthanabsy,_mm256_and_ps(xgreaterthan0,AVX_SIGNMASK))); //negate atan if absx<=absy & x>0

    __m256 shift=_mm256_set1_ps(pi);
    shift=_mm256_sub_ps(shift,_mm256_andnot_ps(absxgreaterthanabsy,_mm256_set1_ps(halfpi))); //substract halfpi if absx<=absy
    shift=_mm256_xor_ps(shift,_mm256_andnot_ps(ygreaterthan0, AVX_SIGNMASK)); //negate shift if y<=0
    shift=_mm256_andnot_ps(_mm256_and_ps(absxgreaterthanabsy,xgreaterthan0),shift); //null if abs>absy & x>0
...
__m256 FTA::cos_52s_ps(__m256 x){
    ...
    __m256 x2;      // The input argument squared
...
__m256 FTA::cos_ps(__m256 angle){
    ...
    //angle=_mm_sub_ps(angle,_mm_mul_ps(_mm_cvtepi32_ps(_mm_cvttps_epi32(_mm_mul_ps(angle,_mm_set1_ps(invtwopi)))),_mm_set1_ps(twopi))); //simplied SSE2 fmod, must always operate on absolute value
    //if SSE4.1 is always available, comment the line above and uncomment the line below
    angle=_mm256_sub_ps(angle,_mm256_mul_ps(_mm256_floor_ps(_mm256_mul_ps(angle,_mm256_set1_ps(invtwopi))),_mm256_set1_ps(twopi))); //faster if SSE4.1 is always available
...
void FTA::sincos_ps(__m256 angle, __m256 *sin, __m256 *cos){
    ...
    *sin=_mm256_mul_ps(sinmultiplier,FTA::sqrt_ps(_mm256_sub_ps(_mm256_set1_ps(1.f),_mm256_mul_ps(result,result))));
...
/*
void FTA::interleave_ps(__m256 x0x1x2x3, __m256 y0y1y2y3, __m256 *x0y0x1y1, __m256 *x2y2x3y3){
    *x0y0x1y1=_mm256_unpacklo_ps(x0x1x2x3,y0y1y2y3);
    *x2y2x3y3=_mm256_unpackhi_ps(x0x1x2x3,y0y1y2y3);
}

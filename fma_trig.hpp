...
__m256 FTA::sqrt_ps(__m256 squared){return _mm256_sqrt_ps(squared);}
__m256d FTA::sqrt_pd(__m256d squared){return _mm256_sqrt_pd(squared);}
...
__m256d FTA::cos_2pi_poly_pd(__m256d x) {
    const __m256d c0 = _mm256_set1_pd(9.99999999999690912008938325207278291e-01);
    const __m256d c1 = _mm256_set1_pd(-1.97392088012971049342846292017770781e+01);
    const __m256d c2 = _mm256_set1_pd(6.49393937659748342821090830188040256e+01);
    const __m256d c3 = _mm256_set1_pd(-8.54567909315223589490695570169307119e+01);
    const __m256d c4 = _mm256_set1_pd(6.02433903394458272530415383043931487e+01);
    const __m256d c5 = _mm256_set1_pd(-2.63960034791736043509175907518263228e+01);
    const __m256d c6 = _mm256_set1_pd(7.54198572657058592766093467814729792e+00);
    __m256d x2 = _mm256_mul_pd(x, x);

    // Using FMA instructions for more efficient computation
    __m256d result = _mm256_fmadd_pd(c6, x2, c5); // c5 + c6 * x^2
    result = _mm256_fmadd_pd(result, x2, c4);    // c4 + (c5 + c6 * x^2) * x^2
    result = _mm256_fmadd_pd(result, x2, c3);    // …
    result = _mm256_fmadd_pd(result, x2, c2);
    result = _mm256_fmadd_pd(result, x2, c1);
    result = _mm256_fmadd_pd(result, x2, c0);

    return result;
}

__m256d FTA::cos_2pi_pd(__m256d angle){
    //clamp to the range 0..1
    ...
    __m256d result=FTA::cos_2pi_poly_pd(cosangle);
...
__m256d FTA::sin_2pi_pd(__m256d angle){
    return FTA::cos_2pi_pd(_mm256_sub_pd(_mm256_set1_pd(0.25),angle));
}


void FTA::sincos_2pi_pd(__m256d angle, __m256d *sin, __m256d *cos){
    ...
    *sin=_mm256_mul_pd(sinmultiplier,FTA::sqrt_pd(_mm256_fnmadd_pd(result, result, _mm256_set1_pd(1.f))));
